...
经与产品确认,本轮采用 **混合链路 + 文件解析分治**:
- **文本类 AI 生成(追问、报告、方向推荐)继续在 App 内直连 DeepSeek**(属「文本」,打包后可独立运行)。
- **PDF / DOCX 解析在 App 端上完成**(2026 年 RN 生态已支持:PDF 用原生模块 `expo-pdf-text-extract`,DOCX 用纯 JS 解 zip+XML),保持「大部分场景 App 独立可用」的初衷。
- **MP3 语音转写走后端云 ASR**(端上跑 Whisper 在中文面试录音场景代价大:APK 体积暴涨、中端机推理慢、中文小模型精度不稳;且云 ASR key 必须留后端)。本轮 MP3 仅留后端接口桩,真实 ASR 选型后续单独议。
- **账号体系走后端**(需鉴权)。

> 打包注意:`expo-pdf-text-extract` 是**原生模块**,依赖 CNG prebuild / EAS 构建,**不能在 Expo Go 中测**(项目已过 Expo Go 阶段,走 `expo run:android` / EAS);DOCX 用纯 JS 库(如 `fflate`)不需原生模块。
...
### Requirement 1 — 多格式面试材料上传(优先做,分治)
...
8. WHEN 后端 MP3 端点收到超过大小上限的文件 THEN 后端 SHALL 拒绝并返回明确错误(防止超大文件拖垮服务)。
9. WHEN PDF 原生模块在当前运行环境不可用(如 Expo Go / web)THEN App SHALL 优雅降级为提示「该格式需在正式 App 内使用,或直接粘贴文本」,不崩溃。
...
- **解析层分治**:PDF/DOCX 端上解析封装在前端 `services/fileExtract/`(每格式一个纯函数);后端 `file_service.py` 本轮只承载 MP3 转写桩,不做 PDF/DOCX。
...
- 后端 MP3 转写端点对单文件大小设上限(建议 ≤ 25MB,后续可调),拒绝超限文件。
- PDF/DOCX 端上解析:大文件解析在主线程外/异步进行,避免卡 UI;文本类 AI 生成继续在端上直连,不因本轮变慢。
...
- 鉴权失败信息不泄露账号是否存在(防枚举)。
- 后端 MP3 端点不回显文件原始路径/服务器内部错误细节给客户端;云 ASR key(将来)只进后端 `.env`。
...
- MP3 仅留后端接口桩(`NotImplementedError` / 明确错误信封),不埋假实现;ASR 选型与接入作为后续独立工作。
- PDF 端上原生模块依赖 CNG/EAS 构建,不在 Expo Go 可用;DOCX 用纯 JS 库无原生依赖。新增前端依赖锁版本。
